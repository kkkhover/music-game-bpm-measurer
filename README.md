...
> 版本：**主程序 v0.8.16** / 内置侧栏 **v0.8.16**（两侧版本号各自独立）
...
- 🡒 **安装包** `BPM 测速助手 Setup 0.8.16.exe`（免安装版请下载后解压）
...
osu-maphelper_v0.8.16/                    osu! 制谱侧栏源码（零第三方运行时依赖）
...
│
├─ 软件\
│   ├─ BPM 测速助手 Setup 0.8.16.exe   ← 安装包（可选安装目录、建快捷方式）
...
- **自动跟随播放头**（v0.8.16，默认开）：播放中播放头跑出可视区时，视图自动平移跟过去
