...
(function () {
    ...
    function render(s) {
        ...
        if (bm) {
            ...
            syncEl.textContent = bm.inSync ? tt('inSync', '同步') : `${bm.redCount}/${bm.memRedCount}`;
            ...
            $('stat-sync').className = '';
            $('path-hint').textContent = s.osu.installDir
                ? tt('osuDir', 'osu! 目录：') + s.osu.installDir
                : tt('noOsuDir', '未找到 osu! 安装目录');
        }
    }
